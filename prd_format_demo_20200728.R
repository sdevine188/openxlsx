...
style_table_2 <- tibble(row_type = c("header", "body", "body", "footnote"), rows_from = c(1, 1, 4, 1), rows_to = c(1, 3, 7, 1), 
                        cols_from = c(1, 1, 1, 1), cols_to = c(1, 2, ncol(table_2), 1),
                        font_name = c("Arial", "Arial", NA, "Arial"), font_size = c(NA, 15, 7, NA), font_color = c(NA, "#cc0099", NA, NA),
...
# create style_table_3 as example of simple style changes
style_table_3 <- tibble(row_type = c("header", "body", "footnote"), 
                        rows_from = rep(1, times = 3), 
                        rows_to = rep(nrow(table_1), times = 3),
                        cols_from = rep(1, times = 3),
                        cols_to = rep(ncol(table_1), times = 3),
                        font_name = NA,
                        font_size = NA,
                        font_color = "#0000ff",
                        border = NA,
...
createWorkbook() %>% prd_format(tables = list(table_1, table_2), output_sheet_names = c("table_1_sheet", "table_2_sheet"),
                                text_cols = list(9, 2), font_size = 10, custom_col_width = NULL, custom_row_height = NULL,
                                style_table = NULL, footnote_table = footnote_table, superscript_table = superscript_table,
                                col_width_padding = 1, min_col_width = 8.43, overwrite = TRUE) %>% openXL()

# output multiple tables, increased col_width_padding, w/ style_table    
previous_workbook <- loadWorkbook(file = "example_table_output.xlsx")
openXL(previous_workbook)
prd_format(workbook = previous_workbook, tables = list(table_1, table_2), output_sheet_names = "placeholder_title", 
           text_cols = list(9, 2), font_size = 9, custom_col_width = NULL, custom_row_height = NULL, 
           style_table = style_table_3, footnote_table = NULL, superscript_table = NULL, 
