...
merge_footnote_rows <- function(current_row, workbook, sheet, table, list_of_preceding_footnotes, ...) {
        ...
        mergeCells(wb = workbook, sheet = sheet, cols = 1:(table %>% ncol()), 
                   rows = table %>% nrow() + 1 + total_preceding_footnote_nrow + current_row)
}
...
# writeData(wb = workbook, sheet = sheet, x = footnote_tbl_1 %>% rename(" " = footnote_text), startRow = table %>% nrow() + 2, startCol = 1)
# writeData(wb = workbook, sheet = sheet, x = footnote_tbl_2 %>% rename(" " = footnote_text), 
#           startRow = table %>% nrow() + 2 + (footnote_tbl_1 %>% nrow()) + 1, startCol = 1)
# writeData(wb = workbook, sheet = sheet, x = footnote_tbl_3 %>% rename(" " = footnote_text), 
#           startRow = table %>% nrow() + 2 + (footnote_tbl_1 %>% nrow()) + 1 + (footnote_tbl_2 %>% nrow() + 1), startCol = 1)
# 
# # inspect
# openXL(workbook)
# 
